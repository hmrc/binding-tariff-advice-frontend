...
class SubmittedAnswers(advice: Advice) extends RetrieveSubmittedAnswersAction(Mockito.mock(classOf[AdviceService])) {
...
object SubmittedAnswers {
  def apply(advice: Advice): SubmittedAnswers = new SubmittedAnswers(advice)
}
